...
def SpamTest(emailData):
    ...
    for i in range(DocumentsNumber):
        doc = open("email/CSDMC2010_SPAM/train_data/TRAIN_%05d.eml" % i).read()
        doc =   dr.sub('',doc)
        Emails.append(doc)
        ...
        ind += 1
    # read the label
    label = open("email/CSDMC2010_SPAM/SPAMTrain.label").read()
    ...
    emailDatadoc = TextParse(emailData)
    vec = CreateBOWVec(Vocabulary, emailDatadoc)
